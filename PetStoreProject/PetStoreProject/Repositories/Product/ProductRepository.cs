    ...
    public class ProductRepository : IProductRepository
    {
        ...
        public List<RelatedProductViewModel> getRelatedProduct(int productId, int customerId)
        {
            ...
            products = (from po in _context.ProductOptions
                        ...
                        join b in _context.Brands on p.BrandId equals b.BrandId
                        join i in _context.Images on po.ImageId equals i.ImageId
                        where p.ProductCateId == cateId && (po.IsSoldOut == false && po.Quantity > 0) && p.IsDelete == false
        ...
        public List<Models.Product> SuggestProduct(int customerId, List<int> categoryIds)
        {
            ...
            string query = $@"
                            ...
                            LEFT JOIN Feedback f 
							  ...
							  left join ProductCategory pc on p.ProductCateId = pc.ProductCateId
							  left join Category ca on pc.CategoryId = ca.CategoryId
                            WHERE ca.CategoryId IN {categoryIdList}
        ...
        {
            var products = await GetProductsByCategoriesAndProductCateId(cateId, productCateId, customerId);
